"""OpenAI GPT-4o-Transcribe REST API client."""
...
class OpenAIError(Exception):
    """Raised on OpenAI API errors (auth, network, quota)."""
...
def get_api_key() -> str | None:
    """Return the OpenAI API key from environment or .env file."""
...
def transcribe(
    ...
    """Send audio to OpenAI and return the transcript text.
    ...
    Args:
        ...
        prompt: Optional prompt to guide spelling (e.g. medical glossary).

    Returns:
        Transcript string.

    Raises:
        OpenAIError: On missing key, HTTP errors, or empty response.
    """
    import httpx
    ...
    try:
        ...
        raise OpenAIError(f"Network error: {e}") from e

    if resp.status_code == 401:
        raise OpenAIError("Invalid OpenAI API key (401 Unauthorized)")
    if resp.status_code == 429:
        raise OpenAIError("OpenAI rate limit exceeded (429)")
    if resp.status_code != 200:
        raise OpenAIError(f"OpenAI API error {resp.status_code}: {resp.text[:200]}")
